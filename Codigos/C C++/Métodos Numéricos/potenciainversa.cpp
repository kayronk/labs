...
double** matrizInversa(double **m, int n){
    double **inv;
    int i,j,w;
    double p, op;
    ...
    for(i=0;i<n;i++){
        ...
        for(w=0;w<n;w++){
            if(w != i){
                op = m[w][i];
                for(j=0;j<n;j++){
                    m[w][j] = m[w][j]-(op*(m[i][j]));
                    inv[w][j] = inv[w][j]-(op*(inv[i][j]));
                }
            }
        }
    }
...
void potenciainversa(double **a, double *v, double e, int n){
    int i;
    double *v1, *v2;
    double **inv;
    double autovalor2;
    ...
    do{
        ...
        for(i=0;i<n;i++){
            v2[i] = multiMatrizVetor(inv,autovetor,n,n,n)[i];
        }
